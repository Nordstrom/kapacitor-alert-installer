...
deleteTasks() { handleError $(kapacitor delete tasks "$1" 2>&1 1> /dev/null) }

deleteTemplates() { handleError $(kapacitor delete templates "$1" 2>&1 1> /dev/null) }
...
getType() { if grep -q "stream" "$1"; then type="stream"; else type="batch"; fi; echo $type }

handleError() { if [[ "$1" == *"err"* || "$1" == *"failed"* || "$1" == *"unknown"* ]]; then exit 1; fi }
